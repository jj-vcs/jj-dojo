...
let extensionAnyApiProposalEnabled: boolean | undefined;

/**
 * vscode.extensions.all only fetches extensions in the same extension host.
 * This only works externally when the icon theme extensions are placed in
 * the same extension host as the JJ extension. Internally we'll need to use
 * the vscode.extensions.allAcrossExtensionHosts proposed api so it fetches
 * extensions across all extension hosts.
 */
function getAllExtensions(): typeof vscode.extensions.all {
  let allExtensions: typeof vscode.extensions.all;

  if (extensionAnyApiProposalEnabled === undefined) {
    try {
      /* eslint-disable  @typescript-eslint/no-explicit-any */
      allExtensions = (vscode.extensions as any).allAcrossExtensionHosts;
      extensionAnyApiProposalEnabled = true;
      return allExtensions;
    } catch (_err: unknown) {
      extensionAnyApiProposalEnabled = false;
    }
  }
  return extensionAnyApiProposalEnabled
    ? /* eslint-disable  @typescript-eslint/no-explicit-any */
      (vscode.extensions as any).allAcrossExtensionHosts
